...
$(out)/BOOTX64.efi:
	$(elfbin)/x86_64-elf-gcc $(src)/uefi.c -c -fno-stack-protector -fpic -fshort-wchar -mno-red-zone -I /usr/include -I /usr/include/efi/x86_64 -I /usr/include/x86_64-linux-gnu -DEFI_FUNCTION_WRAPPER -ffreestanding -masm=intel -o $(out)/main.o
...
$(out)/klos.img:
	...
	mformat -i $(out)/klos.img -F
	mmd -i $(out)/klos.img ::/EFI
	mmd -i $(out)/klos.img ::/EFI/BOOT
	mcopy -i $(out)/klos.img $(out)/BOOTX64.efi ::/EFI/BOOT/
#	dd if=/dev/zero of=$(out)/klos.img bs=512 count=93750
